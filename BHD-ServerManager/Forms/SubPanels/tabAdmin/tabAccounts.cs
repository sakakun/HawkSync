...
public partial class tabAccounts : UserControl
{
    ...
    private readonly string _tickerName = $"AdminAccountsTicker_{Guid.NewGuid():N}";
    ...
    {
        if (_runtimeInitialized || IsDesignTime)
            return;
    ...
    protected override void OnVisibleChanged(EventArgs e)
    {
        ...
        if (Visible)
        {
            CommonCore.Ticker?.Start(_tickerName, 1000, TickerAdminTick);
        }
        else
        {
            CommonCore.Ticker?.Stop(_tickerName);
        }
    }
    ...
    {
        if (IsDesignTime || !_runtimeInitialized)
            return;

        if (Interlocked.CompareExchange(ref _tickerBusy, 1, 0) != 0)
            return;

        if (InvokeRequired)
        {
            BeginInvoke(TickerAdminTick);
            Interlocked.Exchange(ref _tickerBusy, 0);
            return;
        }
