...
namespace BHD_ServerManager.Forms.Panels
{
	public partial class tabGamePlay : UserControl
	{
		...
		public void UpdateServerControls()
		{
			...
			btn_MatchStateToggle.Visible = !isOffline;

			// Update match state toggle button text based on current timer value
			if (!isOffline && theInstance != null)
			{
				try
				{
		...
		private void actionClick_ToggleMatchState(object sender, EventArgs e)
		{
			try
			{
				// Use the current timer value from theInstance (updated by ticker) to determine current match state
				int currentTimer = CommonCore.theInstance?.gameInfoStartDelayTimer ?? 0;
				...
				if(isCurrentlyPaused)
				{
					...
					theInstance!.gameInfoStartDelayTimer = 0; // Update local instance value immediately to reflect change in UI
					btn_MatchStateToggle.Text = "PAUSE MATCH";
				}
				else
				{
					ServerMemory.UpdateStartDelayTimer(240);
					theInstance!.gameInfoStartDelayTimer = 240; // Update local instance value immediately to reflect change in UI
					btn_MatchStateToggle.Text = "RESUME MATCH";
				}
			} catch (Exception ex)
			{
