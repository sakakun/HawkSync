...
namespace BHD_ServerManager.Forms
{
    public partial class ServerManager : Form
    {
        ...
        private void PostServerManagerInitalization(object? sender, EventArgs e)
        {
            ...
            functionEvent_firstTimeLoadPlaylistToDataGride();           // Import the current map playlist to the DataGridView
            // Ban Tab
        ...
        {
            if (dg_slapMessages.InvokeRequired)
            {
                dg_slapMessages.Invoke(new Action(() => functionEvent_UpdateSlapMessages()));
                return;
            }
        ...
        {
            if (dg_autoMessages.InvokeRequired)
            {
                dg_autoMessages.Invoke(new Action(() => functionEvent_UpdateAutoMessages()));
                return;
            }
        ...
        private void functionEvent_PopulateMapDataGrid(DataGridView dgv, IEnumerable<mapFileInfo> maps, bool includeRowNum)
        {
            ...
            {
                dgv.Invoke(new Action(() => functionEvent_PopulateMapDataGrid(dgv, maps, includeRowNum)));
                return;
            }
        ...
        // Scope: MapTab, Function: functionEvent_firstTimeLoadPlaylistToDataGride, Loads the currentMapPlaylist.mpl from the Settings folder and loads it into currentMapPlaylist
        ...
        private void actionClick_startServer(object sender, EventArgs e)
        {
            ...
            if (theInstanceManager.ValidateGameServerPath() && theInstance.instanceStatus == InstanceStatus.OFFLINE)
            {
                ...
                {
                    // If the server started successfully, update the instance status
                    GameManager.ReadMemoryServerStatus();              // Get Server Status
                    // Update the Server Start/Stop Buttons
                    functionEvent_swapFieldsStartStop();                // Trigger Button Change
                }
            }
            else
            {
        ...
        {
            // Run actionClick_saveCurrentMapList to ensure the current map list is saved
        ...
        private async void ActionEvent_TestBabstatConnection(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Failed to connect to Babstats server. Please check the server path and try again.", "Connection Test Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            if (dg_AdminUsers.InvokeRequired)
            {
                dg_AdminUsers.Invoke(new Action(() => functionEvent_UpdateAdminList()));
                return;
            }
