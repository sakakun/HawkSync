...
namespace RemoteClient.Forms.SubPanels
{
	public partial class ChatHistory : UserControl
	{
		...
		private void InitializeControls()
		{
			...
			_ = LoadPlayerFilterAsync();
		}

		/// <summary>
		/// Load distinct player names for the filter dropdown
		/// </summary>
		private async Task LoadPlayerFilterAsync()
		{
			try
			{
			...
			{
				Console.WriteLine($"Failed to load player names: {ex.Message}");
				comboBox_PlayerFilter.Items.Clear();
				comboBox_PlayerFilter.Items.Add("All Players");
				comboBox_PlayerFilter.SelectedIndex = 0;
			}
		...
		/// </summary>
		private async void btn_LoadHistory_Click(object sender, EventArgs e)
		{
			_currentPage = 1;
		...
		/// <summary>
		/// Main method to load chat history from API with filters
		/// </summary>
		private async Task LoadHistoryAsync()
		{
			...
			try
			{
				...
				// Load from API
				var result = await ApiCore.ApiClient!.Chat.GetChatHistoryAsync(request);
				...
				{
					MessageBox.Show(
						"Failed to load chat history from server.",
						"Error",
						MessageBoxButtons.OK,
		...
		/// </summary>
		private async void comboBox_PageSize_SelectedIndexChanged(object sender, EventArgs e)
		{
		...
		/// </summary>
		private async void textBox_Search_KeyPress(object sender, KeyPressEventArgs e)
		{
		...
		/// </summary>
		public async Task RefreshPlayerFilterAsync()
		{
			await LoadPlayerFilterAsync();
		}

		/// <summary>
		/// Public method to trigger a search programmatically (optional)
		/// </summary>
		public async Task SearchChatHistoryAsync(string searchText)
		{
